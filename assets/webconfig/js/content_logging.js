	...
	if (!window.loggingHandlerInstalled)
	{
		...
		$(window.hyperion).on("cmd-logging-update",function(event){
			...
			for(var idx = 0; idx < messages.length; idx++)
			{
				...
				LogLine(idx, $("#logmessages"), date, app_name, logger_name, level_string, debug,msg);
...
function LogLine(line, logger,date,app_name,logger_name,level_string,debug,msg)
{
	var style="";
	...
	if (line !=0)
		logger.append("\n<code"+style+">"+date.toISOString()+" ["+(app_name+" "+logger_name).trim()+"] ("+level_string+") "+debug+msg+"</code>");
	else
		logger.append("<code"+style+">"+date.toISOString()+" ["+(app_name+" "+logger_name).trim()+"] ("+level_string+") "+debug+msg+"</code>");
}