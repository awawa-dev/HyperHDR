...
void SSDPDiscover::setAddress(const QString& address)
{
	_ssdpAddr = QHostAddress(address);
}

void SSDPDiscover::setPort(quint16 port)
{
	_ssdpPort = port;
}

void SSDPDiscover::setMaxWaitResponseTime(int maxWaitResponseTime)
{
	_ssdpMaxWaitResponseTime = maxWaitResponseTime;
}

void SSDPDiscover::setSearchTarget(const QString& searchTarget)
{
	_searchTarget = searchTarget;
}

void SSDPDiscover::clearSearchFilter()
{
	_filter = DEFAULT_FILTER; _filterHeader = "ST";
}

void SSDPDiscover::skipDuplicateKeys(bool skip)
{
	_skipDupKeys = skip;
}
