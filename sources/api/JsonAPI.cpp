...
void JsonAPI::handleMessage(const QString& messageString, const QString& httpAuthHeader)
{
	try
	{
		...
		SAFE_CALL_1_RET(_instanceManager, IsInstanceRunning, bool, isRunning, quint8, currentIndex);
...
void JsonAPI::handleServerInfoCommand(const QJsonObject& message, const QString& command, int tan)
{
	try
	{
		...
		if (!subscribeOnly)
		{
			...
			SAFE_CALL_1_RET(_hyperhdr, getJsonInfo, QJsonObject, info, bool, true);
			...
			if (SoundCapture::getInstance() != NULL)
			{
				...
				SAFE_CALL_0_RET(soundInstance, getJsonInfo, QJsonObject, resultSound);
			...
			{
				QJsonObject systemGrabbers;
				auto systemInstance = SystemWrapper::getInstance();

				SAFE_CALL_0_RET(systemInstance, getJsonInfo, QJsonObject, systemGrabbers);

				info["systemGrabbers"] = systemGrabbers;
			}
#endif
			...
			if (GrabberWrapper::getInstance() != nullptr)
				SAFE_CALL_0_RET((GrabberWrapper::getInstance()), getJsonInfo, QJsonObject, grabbers);
#endif
...
void JsonAPI::handleSaveDB(const QJsonObject& message, const QString& command, int tan)
{
	if (_adminAuthorized)
	{
		...
		SAFE_CALL_0_RET(_instanceManager, getBackup, QJsonObject, backup);
...
void JsonAPI::handleLoadDB(const QJsonObject& message, const QString& command, int tan)
{
	if (_adminAuthorized)
	{
		...
		SAFE_CALL_1_RET(_instanceManager, restoreBackup, QString, error, QJsonObject, message);
...
void JsonAPI::handleLoadSignalCalibration(const QJsonObject& message, const QString& command, int tan)
{
	...
	if (subcommand == "start")
	{
		if (_adminAuthorized)
		{
			SAFE_CALL_0_RET((GrabberWrapper::getInstance()), startCalibration, QJsonDocument, retVal);
	...
	{
		SAFE_CALL_0_RET((GrabberWrapper::getInstance()), stopCalibration, QJsonDocument, retVal);
		sendSuccessDataReply(retVal, full_command, tan);
	}
	else if (subcommand == "get-info")
	{
		SAFE_CALL_0_RET((GrabberWrapper::getInstance()), getCalibrationInfo, QJsonDocument, retVal);
		sendSuccessDataReply(retVal, full_command, tan);
	}
	else
		sendErrorReply("Unknown subcommand", command, tan);
...
void JsonAPI::handleLedColorsCommand(const QJsonObject& message, const QString& command, int tan)
{
	...
	if (subcommand == "ledstream-start")
	{
		...
		QUEUE_CALL_0(_hyperhdr, requestForColors);
	}
...
*/ {
		...
		else if (subc == "identify")
		{
			...
			{
				QUEUE_CALL_1(_hyperhdr, identifyLed, QJsonObject, params);
			}
			else
			{
				ledDevice = LedDeviceFactory::construct(config);
...
void JsonAPI::handleInstanceStateChange(InstanceState state, quint8 instance, const QString& name)
{
	switch (state)
	{
	case InstanceState::H_PRE_DELETE:
...
void JsonAPI::handleSysInfoCommand(const QJsonObject&, const QString& command, int tan)
{
	...
	bool readOnly = true;
	SAFE_CALL_0_RET(_hyperhdr, getReadOnlyMode, bool, readOnly);
...
void JsonAPI::handleAdjustmentCommand(const QJsonObject& message, const QString& command, int tan)
{
	...
	// commit the changes
	BLOCK_CALL_0(_hyperhdr, adjustmentsUpdated);
