...
public:
	...
	void process(const Image<Pixel_T>& image, std::vector<ColorRgb>& ledColors)
	{
		if ( image.width()>0 && image.height()>0)
		{
			...
			_imageToLeds->Process(image, ledColors, advanced);			
		}
		else
		{
...
private:
	...
	void verifyBorder(const Image<Pixel_T> & image)
	{
		if (!_borderProcessor->enabled() && ( _imageToLeds->horizontalBorder()!=0 || _imageToLeds->verticalBorder()!=0 ))
		{
			...
			delete _imageToLeds;
			_imageToLeds = new hyperion::ImageToLedsMap(_mappingType, image.width(), image.height(), 0, 0, _ledString.leds());
		}
		...
		if(_borderProcessor->enabled() && _borderProcessor->process(image))
		{
			...
			{
				// Construct a new buffer and mapping
				_imageToLeds = new hyperion::ImageToLedsMap(_mappingType, image.width(), image.height(), border.horizontalSize, border.verticalSize, _ledString.leds());
			}
