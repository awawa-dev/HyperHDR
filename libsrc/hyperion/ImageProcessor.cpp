...
void ImageProcessor::setSize(unsigned width, unsigned height)
{
	...
	_imageToLeds = (width>0 && height>0) ? (new ImageToLedsMap(_mappingType, width, height, 0, 0, _ledString.leds())) : nullptr;
}
...
void ImageProcessor::setLedString(const LedString& ledString)
{
	if ( _imageToLeds != nullptr)
	{
		...
		// Construct a new buffer and mapping
		_imageToLeds = new ImageToLedsMap(_mappingType, width, height, 0, 0, _ledString.leds());
	}
}
